;(function() {
  UploadCare.DragDrop = {
    ...
    createDropZone: function ($, widget) {
      var cropper, dropZoneContainer, dropZoneNode;
      ...
      dropZoneContainer.html('<span>Drop files here</span>');
    ...
    process: function (widget, hidden, files) {
      ...
      xhr.open("POST", UploadCare.byIframe.uploadUrl, true);
      xhr.send(form);
    ...
    onDragOver: function (ev, dragzone, dropzone) {
      ...
      dragzone.parents(this.selectors.widget).find(this.selectors.filedata).css({
        top: -1
      });
    },
...
UploadCare.ready(function($) {
  $(UploadCare.Line).bind('enlive', function() {
